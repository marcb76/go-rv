...
| **`/api/url`** | `POST` | **Creation & AI Enrichment:** Accepts a long URL, triggers an integration with the Gemini API to generate `aiTags` and `aiDescription`, allocates a unique code, persists the record, and returns the complete `URLRecord` in the response body. |
| **`/api/url`** | `GET` | **Global Audit:** Returns the complete in-memory map containing all registered items and their associated metadata in JSON format. |
| **`/api/url/{code}`** | `GET` | **Individual Query:** Looks up a specific record by its unique `code` key and returns its `URLRecord` in JSON format. |
